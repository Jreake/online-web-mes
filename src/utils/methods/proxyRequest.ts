			...
			caller(...args)
				...
				.finally(() => {
					...
					if (this._taskQueue.length) {
						...
						let task: any = this._taskQueue.shift();
						task();
					} else {
...
function request1(i) {
	return new Promise(resolve => {
		console.log("request1" + JSON.stringify(i));
		setTimeout(() => {
...
function request2(i) {
	return new Promise(resolve => {
		console.log("request2" + JSON.stringify(i));
		setTimeout(() => {
			resolve(JSON.stringify(i));
		}, 4000);
	});
...
for (let i = 0; i < 11; i++) {
	if (i % 2 == 0) {
		limitP.call(request1, { i }).then(res => {
			console.log(res);
		});
	} else {
		limitP.call(request2, { i }).then(res => {
			console.log(res);
		});
	}
}
