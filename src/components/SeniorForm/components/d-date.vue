...
// 引入表单子控件的管理类
import formItemManage from "@/components/controlManage/formItemManage";
// 引入组件需要的属性
import { baseFormProps } from "@/components/controlConfig/formItemMeta";
	...
	if (controlType === 113) {
		...
		if (arr.length > 1) {
			mydate.value = new Date(
				new Date(arr[0] + "-1-1").valueOf() + arr[1] * 7 * 24 * 3600000
			);
		}
	}
